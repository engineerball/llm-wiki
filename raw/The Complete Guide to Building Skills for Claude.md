...
At Anthropic, three common use cases have been observed:
...
description: End-to-end customer onboarding workflow for PayFlow.
  ...
  management. Use when user says "onboard new customer",
...
Recommended structure (adapt this template, replace bracketed sections):
...
Use the metrics from "Define Success Criteria." Example comparison:
...
Anthropic has published Agent Skills as an open standard. Like MCP, skills should be portable across tools and platforms - the same skill should work whether you're using Claude or other AI platforms. Some skills are designed to take full advantage of a specific platform's capabilities; authors can note this in the skill's `compatibility` field. Anthropic has been collaborating with members of the ecosystem on the standard.

### Using skills via API

For programmatic use cases - building applications, agents, or automated workflows that leverage skills - the API provides direct control over skill management and execution.
...
Start by hosting your skill on GitHub with a public repo, clear README (for human visitors — separate from your skill folder, which should not contain a README.md), and example usage with screenshots. Then add a section to your MCP documentation that links to the skill, explains why using both together is valuable, and provides a quick-start guide.
...
These patterns emerged from skills created by early adopters and internal teams. They represent common approaches that have worked well, not prescriptive templates.
...
- Cause: YAML formatting issue (missing `---` delimiters, unclosed quotes, etc.)

Error: "Invalid skill name"
- Cause: Name has spaces or capitals (use kebab-case)
...
Solutions:
1. Add negative triggers: `description: Advanced data analysis for CSV files. Use for statistical modeling, regression, clustering. Do NOT use for simple data exploration (use data-viz skill instead).`
2. Be more specific: "Processes documents" → "Processes PDF legal documents for contract review"
3. Clarify scope: name the specific service/domain and exclude general queries.
...
4. Verify tool names are correct and case-sensitive per MCP server documentation
...
1. Instructions too verbose — keep concise, use bullet points/numbered lists, move detail to reference files
2. Instructions buried — put critical instructions at the top, use `## Important` or `## Critical` headers, repeat key points if needed
3. Ambiguous language — replace "Make sure to validate things properly" with explicit CRITICAL checklists
   - Advanced technique: for critical validations, bundle a script that performs the checks programmatically rather than relying on language instructions. Code is deterministic; language interpretation isn't. See the Office skills for examples.
4. Model "laziness" — add explicit encouragement ("Take your time to do this thoroughly", "Quality is more important than speed", "Do not skip validation steps"). Note: adding this to user prompts is more effective than in SKILL.md.
...
For Technical Questions: General questions at the Claude Developers Discord (community forums).

For Bug Reports: GitHub Issues: anthropics/skills/issues — include skill name, error message, steps to reproduce.
